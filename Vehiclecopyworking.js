class Vehicle {
    ...
    animate = ()=> {
        ...
        if (this.elapsed > this.fpsInterval && this.intNo < this.noOfPoints) {
            ...
            fetch(`https://api.mapbox.com/v4/johnpaul0220.2gdjokli/tilequery/${this.JsonGeoData.features[0].geometry.coordinates[this.intNo]}.json?access_token=pk.eyJ1Ijoiam9obnBhdWwwMjIwIiwiYSI6ImNrcWZxOW5mMjB0dDEycXBndjVxb2xiamwifQ.zxeqPGE4aSmVgLiJva_pdg`)
                .then(response => response.json())
                .then(data => {
                    if (data.features.length != 0) {
                        this.junction =  this.junctionList.filter(entry =>{
                            if(entry.junctioName == data.features[0].properties.junciton){
                                return entry
                            }
                        })
            ...
            if (this.intNo <= this.noOfPoints && !this.pause) {
               ...
               // map.getSource(this.routeName + 'Line').setData(this.linePoints);
                map.getSource(this.routeName).setData({
                    ...
                    "geometry": {
                        "type": "Point",
                        "coordinates": this.JsonGeoData.features[0].geometry.coordinates[this.intNo]
                    }
                });
                map.panTo(this.JsonGeoData.features[0].geometry.coordinates[this.intNo])
                //console.log('psd')
                this.intNo++
            }
        }
